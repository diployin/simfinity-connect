...
export function NewFooter() {
  ...
  const targetDestinations = [
    { code: 'CHN', label: t('website.destinations.china', 'China') },
    { code: 'JPN', label: t('website.destinations.japan', 'Japan') },
    { code: 'TH', label: t('website.destinations.thailand', 'Thailand') },
    { code: 'AU', label: t('website.destinations.australia', 'Australia') },
    { code: 'USA', label: t('website.destinations.usa', 'United States') },
  ...
  return (
    <footer className="bg-background border-t border-border dark:bg-gray-950">
...
function PaymentIcon({ name }: { name: string }) {
  ...
  switch (name) {
    case 'Apple Pay':
      return <SiApplepay className={`${iconClass} text-black dark:text-white`} />;
    case 'Google Pay':
      return <SiGooglepay className={`${iconClass} text-[#5F6368] dark:text-gray-300`} />;
    case 'Visa':
      return <SiVisa className={`${iconClass} text-[#1434CB]`} />;
    case 'Mastercard':

...
export default function TermsConditionsManagement() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiRequest('GET', '/api/admin/terms-conditions');
      return res.json();
    },
  });

  const termsList: Terms[] = data?.data || [];
  const activeTerms = termsList.find((t) => t.isActive);
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      return apiRequest('POST', '/api/admin/terms-conditions', {
        title,
        content,
      });
    },
    onSuccess: () => {
      ...
      toast({
        title: 'Success',
        description: 'Terms & Conditions updated (new version created)',
      });
    },
    onError: (error: any) => {
      toast({
        title: 'Error',
        description: error.message || 'Failed to update Terms & Conditions',
        variant: 'destructive',
      });
    },
  });
    ...
    <div className="p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Manage your website Terms & Conditions (versioned)
        </p>
      </div>
                  ...
                  {termsList.map((t) => (
                    <div
                      ...
                      className={`p-3 rounded border flex justify-between items-center ${
                        t.isActive ? 'border-green-500 bg-green-50' : 'border-muted'
                      }`}
                    >
                      <div>
                        <p className="font-medium">
                          Version {t.version} {t.isActive && '(Active)'}
                        </p>

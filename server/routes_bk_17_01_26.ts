...
import { db } from "./db";
import { orders, providers, unifiedPackages, reviews, users, insertReviewSchema, referralProgram, referrals, referralSettings, referralTransactions, insertReferralProgramSchema, insertReferralSchema, insertReferralSettingsSchema, blogPosts, airaloTopups, esimAccessTopups, esimGoPackages, esimGoTopups, mayaTopups, destinations, regions } from "@shared/schema";
...
async function awardReferralCredits(userId: string, orderId: string, orderAmount: number) {
  try {
    ...
    const referrer = await db.select({ referralBalance: users.referralBalance })
      .from(users)
    ...
    console.log(`[Referral] Awarded ${rewardAmount} credits to referrer ${referrerId} for order ${orderId}`);
...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      if (normalizedPayload.type === "order_status") {
        ...
        if (batchOrders.length === 0) {
          console.error(`❌ No orders found for webhook (requestId: ${normalizedPayload.requestId}, providerOrderId: ${normalizedPayload.providerOrderId})`);
          return res.status(200).json({ success: true, message: "Webhook received - no orders found" });
        }
        ...
        console.log(`✅ Webhook processed: ${batchOrders.length} orders updated (${provider_slug})`);
        return ApiResponse.success(res, "Webhook processed successfully", { ordersUpdated: batchOrders.length });
      }
        ...
        console.log(`✅ Low data notification processed for ${normalizedPayload.iccid} (${provider_slug})`);
  ...
  app.post("/api/webhooks/airalo/order-complete", async (req: Request, res: Response) => {
    try {
      ...
      console.log(`✅ Batch order updated successfully from webhook. ${batchOrders.length} eSIMs processed.`);
      ApiResponse.success(res, "Webhook processed successfully", { esimsProcessed: batchOrders.length });
    } catch (error: any) {
  ...
  app.post("/api/webhooks/airalo/low-data", async (req: Request, res: Response) => {
    ...
    try {
      ...
      const settingKey = threshold === "75_percent" ? "notif_low_data_75"
        : threshold === "90_percent" ? "notif_low_data_90"
          : threshold === "3_days" ? "notif_expiring_3days"
            : threshold === "1_day" ? "notif_expiring_1day"
              : null;
      ...
      const notificationTitle = threshold === "75_percent" ? "75% Data Used"
        : threshold === "90_percent" ? "90% Data Used"
          : threshold === "3_days" ? "3 Days Remaining"
            : threshold === "1_day" ? "1 Day Remaining"
              : "Low Data Alert";
    ...
    try {
      ...
      if (type === "voucher") {
        ...
        return res.json({
          ...
          description: voucher.description || `${voucher.type === "percentage" ? voucher.value + "%" : "$" + voucher.value} off`,
        });
        ...
        // Validate referral code (user's referral code to get discount as new user)
        ...
        const referralSettings = await storage.getSetting("referral_settings");
        if (!referralSettings?.value) {
          return res.status(400).json({
            success: false,
        ...
        const settings = typeof referralSettings.value === "string"
          ? JSON.parse(referralSettings.value)
          : referralSettings.value;
        ...
        // Calculate referral discount (for the new user using the code)
        const discountType = settings.refereeRewardType || "fixed";
        const discountValue = parseFloat(settings.refereeRewardValue || "0");
        ...
        return res.json({
          ...
          discount: Math.round(discount * 100) / 100,
          description: `Referral discount: ${discountType === "percentage" ? discountValue + "%" : "$" + discountValue} off`,
        });
  ...
  app.post("/api/guest/create-payment-intent", async (req: Request, res: Response) => {
    try {
      ...
      if (!packageId || !email) {
        return res.status(400).json({ success: false, message: "Package ID and email are required" });
      }
      ...
      if (promoCode && promoType) {
        ...
        console.log(`[Guest Checkout] Promo ${promoType} ${promoCode}: ${validatedPromoDiscount.toFixed(2)} ${requestedCurrency} discount`);
      }
      ...
      if (referralCredits > 0 && (req as any).session?.userId) {
        ...
        if (user?.referralBalance) {
          ...
          console.log(`[Guest Checkout] Referral credits applied: ${appliedCredits} ${requestedCurrency}`);
        }
      }
      ...
      // Final Total
      // ----------------------------
      const total = Math.max(subtotal - discount, 0.50);   // minimum 0.50 in selected currency
  ...
  app.post("/api/guest/confirm-payment", async (req: Request, res: Response) => {
    try {
      ...
      // SECURITY: Use metadata from the payment intent, not from client request
      // This prevents hijacking by clients passing arbitrary email/phone
      ...
      try {
        if (pkg.isUnified && pkg.providerId) {
          ...
          console.log(`[Guest Order] Provisioning ${quantity} eSIM(s) via OrderingEngine for order ${order.id}`);
          ...
          if (orderResult.failoverUsed) {
            console.log(`[Guest Order] Failover used for order ${order.id}: ${orderResult.originalProviderId} → ${orderResult.finalProviderId}`);
          }
          ...
          if (usedCredits > 0 && metadata.userId) {
            try {
              ...
              if (creditUser && creditUser.referralBalance) {
                ...
                console.log(`[Guest Order] Deducted $${usedCredits} referral credits from user ${metadata.userId}`);
              }
          ...
          if (metadata.promoType && metadata.promoCode) {
            try {
              if (metadata.promoType === "voucher" && metadata.voucherId) {
                ...
                console.log(`[Guest Order] Voucher ${metadata.promoCode} used for order ${order.id}`);
                ...
                if (giftCard) {
                  ...
                  console.log(`[Guest Order] Gift card ${metadata.promoCode} used: $${promoDiscount} deducted, new balance: $${newBalance.toFixed(2)}`);
                }
              }
          ...
          console.log(`[Guest Order] Package ${pkg.id} missing provider info, order ${order.id} left in processing`);
        }
  ...
  app.post("/api/create-payment-intent", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (promoCode && promoType) {
        ...
        discount += validatedPromoDiscount;
        console.log(`[User Checkout] Promo ${promoType} ${promoCode}: ${validatedPromoDiscount.toFixed(2)} ${requestedCurrency} discount`);
      }
      ...
      if (referralCredits > 0 && userId) {
        ...
        if (user?.referralBalance) {
          ...
          console.log(`[User Checkout] Referral credits applied: ${appliedCredits} ${requestedCurrency}`);
        }
      }
      ...
      res.status(500).json({ success: false, message: "Error creating payment intent: " + error.message });
    }
  });
  ...
  app.post("/api/create-topup-payment-intent", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (!packageId || !iccid || !orderId) {
        return res.status(400).json({ success: false, message: "packageId, iccid, and orderId are required" });
      }
  ...
  app.post("/api/confirm-payment", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (metadata.type === "package_purchase") {
        ...
        // Process order (same logic as existing POST /api/orders)
        const orderType = quantity === 1 ? "single" : "batch";
        ...
        if (orderType === "single") {
          ...
          try {
            ...
            if (pkg.isUnified && pkg.providerId && pkg.providerPackageTable && pkg.providerPackageId) {
              ...
              const airaloResponse = await airaloOrderService.submitSingleOrder(pkg.airaloId, 1, `Order ${order.id}`);
...
${urls.map(url => `  <url>
  ...
  app.get("/api/search", async (req: Request, res: Response) => {
    try {
      ...
      if (q && typeof q === "string" && q.trim()) {
        conditions.push(
          or(
            ilike(unifiedPackages.title, `%${q}%`),
            ilike(unifiedPackages.operator, `%${q}%`)
          )
        );
      }
  ...
  async function generateReferralCode(userName: string): Promise<string> {
    ...
    while (attempts < maxAttempts) {
      ...
      const existing = await db.select().from(referralProgram).where(eq(referralProgram.referralCode, code)).limit(1);
  ...
  app.get("/api/referrals/my-program", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      let program = await db.select().from(referralProgram).where(eq(referralProgram.userId, userId)).limit(1);
      ...
      if (program.length === 0) {
        ...
        const newProgram = await db.insert(referralProgram).values({
          userId,
          referralCode
        }).returning();
  ...
  app.get("/api/referrals/my-referrals", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const totalCount = await db.select({ count: count() }).from(referrals).where(eq(referrals.referrerId, userId));
  ...
  app.get("/api/referrals/my-balance", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const user = await db.select({ referralBalance: users.referralBalance }).from(users).where(eq(users.id, userId)).limit(1);
  ...
  app.post("/api/admin/orders/:id/send-instructions", requireAdmin, async (req: Request, res: Response) => {
    try {
      const order = await storage.getOrderById(req.params.id);
      if (!order) {
        return res.status(404).json({ success: false, message: "Order not found" });
  ...
  app.post("/api/admin/orders/esim", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      if (!provider.enabled) {
        return res.status(400).json({ success: false, message: `Provider ${provider.name} is currently disabled` });
      }
  ...
  app.post("/api/admin/orders/:id/assign", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      if (order.userId !== null) {
        return res.status(400).json({ success: false, message: "eSIM is already assigned to a customer" });
      }

      if (order.status !== "completed") {
        return res.status(400).json({ success: false, message: "eSIM is not ready for assignment. Status: " + order.status });
      }
  ...
  app.get("/api/admin/packages", requireAdmin, async (req, res) => {
    try {
      ...
      const { data, total } =
        await storage.getUnifiedPackagesPaginated({
          limit,
          offset,
          search,
          providerId,
      ...
      const result = await Promise.all(
        data.map(async (pkg) => {
          const destination = pkg.destinationId
            ...
            : null;
          const region = pkg.regionId
            ? await storage.getRegionById(pkg.regionId)
            : null;
  ...
  app.put("/api/admin/packages/:id/custom", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const updatedPkg = await storage.updatePackage(req.params.id, {
        ...
        customDescription: customDescription !== undefined ? customDescription : existingPkg.customDescription,
      });
      ApiResponse.success(res, "Package updated successfully", updatedPkg);
    } catch (error: any) {
      res.status(500).json({ success: false, message: error.message });
    ...
    try {
      ...
      const weightSettings = await db.query.platformSettings.findMany({
        where: inArray(platformSettings.key, [
          "ai_price_weight", "ai_quality_weight", "ai_provider_weight"
        ]),
      });
  ...
  app.post("/api/admin/ai-settings/run-selection", requireAdmin, async (req: Request, res: Response) => {
    try {
      const { packageAutoSelector } = await import("./services/packages/package-auto-selector");
  ...
  app.post("/api/admin/providers/price-comparison", requireAdmin, async (req: Request, res: Response) => {
    try {
  ...
  app.post("/api/admin/providers/:id/sync-topups", requireAdmin, async (req: Request, res: Response) => {
    try {
      const provider = await storage.getProviderById(req.params.id);
  ...
  app.put("/api/admin/provider-priorities", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      await logAdminActivity(
        ...
        null,
        null,
        { action: "reorder_priorities", priorities }
      );
  ...
  app.get("/api/admin/unified-packages", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const bestPriceFilter = req.query.bestPrice === 'true' ? true : req.query.bestPrice === 'false' ? false : null;
  ...
  app.patch("/api/admin/unified-packages/:id", requireAdmin, async (req: Request, res: Response) => {
    try {
  ...
  app.get("/api/admin/master-topups", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const esimAccessWhereClause = esimAccessConditions.length > 0 ? and(...esimAccessConditions) : undefined;
  ...
  app.post("/api/admin/kyc/:id/reject", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const { logActivity, ActivityActions } = await import("./middleware/activity-logger");
      await logActivity(req, ActivityActions.KYC_REJECTED, 'kyc', doc.id, { userId: doc.userId, reason });
  ...
  app.post("/api/reviews", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (!validatedData.comment || validatedData.comment.length < 50) {
        return res.status(400).json({ success: false, message: "Comment must be at least 50 characters" });
      }

      // Check if user has already reviewed this package
      const existingReview = await db.query.reviews.findFirst({
        where: and(
          eq(reviews.userId, req.session.userId!),
          eq(reviews.packageId, validatedData.packageId)
        ),
      });
  ...
  app.put("/api/reviews/:id", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (review.isApproved && review.createdAt < sevenDaysAgo) {
        return res.status(403).json({ success: false, message: "Cannot edit approved review older than 7 days" });
      }
  ...
  app.post("/api/admin/reviews/:id/approve", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      res.json({ success: true, message: "Review approved successfully", data: { review: updatedReview } });
    } catch (error: any) {
      res.status(500).json({ success: false, message: error.message });
    }
  });
  ...
  app.get("/api/esims/:iccid/usage", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      // console.log("Provider service:", JSON.stringify(providerService));
      const providerResponse = await providerService.getUsageData(order.iccid)

      res.json({ usage: providerResponse });
    } catch (error: any) {
  ...
  app.get("/api/esims/:iccid/topup-packages", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      console.log("[Topup Packages] Fetching for ICCID:", req.params.iccid, "Provider:", order.providerId);
  ...
  app.post("/api/topups", requireAuthOrAdmin, async (req: Request, res: Response) => {
    try {
      ...
      if (!orderId || !packageId || !iccid) {
        return res.status(400).json({ success: false, message: "orderId, packageId, and iccid are required" });
      }
  ...
  app.post("/api/admin/orders/:orderId/refresh-status", requireAdmin, async (req: Request, res: Response) => {
    try {
  ...
  app.get("/api/esims/:iccid/branded-qr", requireAuthOrAdmin, async (req: Request, res: Response) => {
    try {
      const { brandName } = req.query;

      const { esimService } = await import("./services/esim");
      const qrData = await esimService.getBrandedQRCode(req.params.iccid, brandName as string | undefined);
  ...
  app.get("/api/admin/notifications/settings", requireAdmin, async (req: Request, res: Response) => {
    try {
      const allSettings = await storage.getAllSettings();
  ...
  app.put("/api/admin/notifications/settings", requireAdmin, async (req: Request, res: Response) => {
    try {
  ...
  app.post("/api/admin/notifications/opt-out", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      if (success) {
        ...
        res.json({ success: true, message: "Successfully opted-out from Airalo low data notifications" });
      } else {
  ...
  app.post("/api/admin/notifications/test-webhook", requireAdmin, async (req: Request, res: Response) => {
    try {
  ...
  app.get("/api/admin/notifications/history", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      if (!source || source === 'all' || source === 'airalo') {
        ...
        allNotifications.push(...airaloNotifs.map(n => ({
          id: n.id,
      ...
      if (!source || source === 'all' || source === 'custom') {
        if (type === 'custom' || !type || type === 'all') {
          ...
          // Transform to unified format
          allNotifications.push(...customNotifs.map(n => ({
            id: n.id,
            source: 'custom' as const,
            type: 'custom',
            iccid: n.recipientType === 'single' ? `Single: ${n.recipientUserId}` : 'All Customers',
  ...
  app.post("/api/admin/notifications/send-custom", requireAdmin, async (req: Request, res: Response) => {
    try {
      const { subject, message, recipientType, recipientUserId, sendEmail: shouldSendEmail = true, sendInApp = true } = req.body;

      if (!subject || !message || !recipientType) {
        return res.status(400).json({ success: false, message: "Subject, message, and recipientType are required" });
      }

      if (recipientType !== 'all' && recipientType !== 'single') {
        return res.status(400).json({ success: false, message: "recipientType must be 'all' or 'single'" });
      }

      if (recipientType === 'single' && !recipientUserId) {
        return res.status(400).json({ success: false, message: "recipientUserId is required when recipientType is 'single'" });
      }

      if (!shouldSendEmail && !sendInApp) {
        return res.status(400).json({ success: false, message: "At least one delivery method (email or in-app) must be selected" });
      }
      ...
      try {
        if (recipientType === 'single') {
          ...
          if (shouldSendEmail) {
            const emailContent = await generateCustomNotificationEmail(subject, message, user.name || user.email);
            try {
              await sendEmail({
                to: user.email,
          ...
          const promises = users.map(async (user) => {
            ...
            if (sendInApp) {
              try {
                ...
                console.error("Failed to create in-app notification for user:", user.email, error);
              }
            }
        ...
        await storage.updateCustomNotification(notification.id, {
          status: 'failed',
          emailsFailed: recipientType === 'single' ? 1 : (await storage.getAllUserEmails()).length,
        });
        throw error;
      }
    } catch (error: any) {
  ...
  app.get("/api/admin/notifications/custom-history", requireAdmin, async (req: Request, res: Response) => {
    try {
  ...
  app.get("/api/admin/email-templates/:eventType", requireAdmin, async (req: Request, res: Response) => {
    try {
  ...
  app.get("/api/admin/referrals", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const totalCountResult = await db.select({ count: count() }).from(referrals).where(whereClause);
      const totalCount = totalCountResult[0]?.count || 0;

      const pendingCount = await db.select({ count: count() }).from(referrals).where(eq(referrals.status, 'pending'));
      const completedCount = await db.select({ count: count() }).from(referrals).where(eq(referrals.status, 'completed'));

      const totalRewardsResult = await db.select({
        total: sql<string>`COALESCE(SUM(${referrals.rewardAmount}), 0)`
      }).from(referrals).where(and(eq(referrals.status, 'completed'), eq(referrals.rewardPaid, true)));

      const pendingPayoutsResult = await db.select({
        total: sql<string>`COALESCE(SUM(${referrals.rewardAmount}), 0)`
      }).from(referrals).where(and(eq(referrals.status, 'completed'), eq(referrals.rewardPaid, false)));
  ...
  app.post("/api/admin/referrals/:id/mark-paid", requireAdmin, async (req: Request, res: Response) => {
    try {
  ...
  app.get("/api/admin/referrals/analytics", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const completedReferralsResult = await db.select({ count: count() }).from(referrals).where(eq(referrals.status, 'completed'));
      ...
      const avgRewardResult = await db.select({
  ...
  app.get("/api/blog", async (req: Request, res: Response) => {
    try {
      ...
      const totalResult = await db.select({ count: count() }).from(blogPosts).where(eq(blogPosts.published, true));
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const metaKeywords = typeof data.metaKeywords === "string"
  ...
  app.post("/api/enterprise/quotes/:id/accept", requireAuth, async (req: Request, res: Response) => {
    try {
      const { bulkQuotes, bulkOrders, enterpriseAccounts, insertBulkOrderSchema } = await import("@shared/schema");
      ...
      if (!account || account.status !== "approved") {
        return res.status(403).json({ success: false, message: "Enterprise account not approved" });
      }
  ...
  app.post("/api/admin/enterprise/accounts", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const [account] = await db.insert(enterpriseAccounts)
        .values(data)
        .returning();
  ...
  app.post("/api/admin/enterprise/accounts/:id/approve", requireAdmin, async (req: Request, res: Response) => {
    try {
      const { enterpriseAccounts } = await import("@shared/schema");
  ...
  app.put("/api/admin/enterprise/accounts/:id", requireAdmin, async (req: Request, res: Response) => {
    try {
      const { enterpriseAccounts } = await import("@shared/schema");
  ...
  app.put("/api/admin/enterprise/quotes/:id", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const [updated] = await db.update(bulkQuotes)
        .set({
          ...req.body,
          updatedAt: new Date(),
        })
  ...
  // Delete bulk quote
  app.delete("/api/admin/enterprise/quotes/:id", requireAdmin, async (req: Request, res: Response) => {
    try {
      const { bulkQuotes } = await import("@shared/schema");

      const [deleted] = await db.delete(bulkQuotes)
        .where(eq(bulkQuotes.id, req.params.id))
        .returning();
  ...
  app.post("/api/admin/enterprise/quotes/:id/send", requireAdmin, async (req: Request, res: Response) => {
    try {
      const { bulkQuotes, enterpriseAccounts } = await import("@shared/schema");
      ...
      if (!quote || !quote.enterpriseAccount) {
        return res.status(404).json({ success: false, message: "Quote or enterprise account not found" });
      }
  ...
  app.post("/api/admin/enterprise/orders", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      if (quote.status !== "approved") {
        return res.status(400).json({ success: false, message: "Quote must be approved before creating order" });
      }
      ...
      const [order] = await db.insert(bulkOrders)
        .values(orderData)
        .returning();
  ...
  app.get("/api/admin/enterprise/orders/:id/details", requireAdmin, async (req: Request, res: Response) => {
    try {
  ...
  app.post("/api/admin/enterprise/orders/:id/execute", requireAdmin, async (req: Request, res: Response) => {
    try {
      const { bulkOrders, bulkQuotes, orders: ordersTable } = await import("@shared/schema");
      const { id } = req.params;
      ...
      if (bulkOrder.status !== "pending") {
        return res.status(400).json({ success: false, message: "Bulk order has already been executed or cancelled" });
      }
      ...
      try {
        ...
        if (quantity > 1 && fullPackage.providerId) {
          ...
          try {
            ...
            const wholesalePricePerEsim = parseFloat(fullPackage.wholesalePrice || quote.unitPrice);
            ...
            console.log(`✅ Created ${orderRecords.length} batch order records for bulk order ${id}`);
          } catch (batchError) {
            ...
            console.log(`✅ Created ${orderRecords.length} individual order records for bulk order ${id}`);
          }
        } else {
  ...
  app.get("/api/admin/enterprise/analytics", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const approvedAccounts = await db.select({ count: count() })
        .from(enterpriseAccounts)
      ...
      const topAccounts = await db.select({
  ...
  app.post("/api/enterprise/register", async (req: Request, res: Response) => {
    try {
      ...
      if (!email || !password || !invitationCode) {
        return res.status(400).json({ success: false, message: "Email, password, and invitation code are required" });
      }
      ...
      if (!enterpriseAccount || enterpriseAccount.status !== "approved") {
        return res.status(400).json({ success: false, message: "Invalid invitation code or account not approved" });
      }
      ...
      const [newUser] = await db.insert(enterpriseUsers).values({
  ...
  app.post("/api/enterprise/login", async (req: Request, res: Response) => {
    try {
      ...
      await db.update(enterpriseUsers)
  ...
  app.post("/api/enterprise/logout", requireEnterpriseAuth, async (req: Request, res: Response) => {
    try {
      ...
      await db.update(enterpriseSessions)
  ...
  app.post("/api/enterprise/quotes/:id/accept", requireEnterpriseAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (quote.status !== "sent") {
        return res.status(400).json({ success: false, message: "Quote is not available for acceptance" });
      }
      ...
      const [order] = await db.insert(bulkOrders).values({
        enterpriseAccountId: quote.enterpriseAccountId,
        quoteId: quote.id,
        totalAmount: quote.totalPrice,
        status: "pending",
  ...
  app.get("/api/enterprise/orders/:id/details", requireEnterpriseAuth, async (req: Request, res: Response) => {
    try {
  ...
  app.post("/api/enterprise/orders/:id/distribute", requireEnterpriseAuth, async (req: Request, res: Response) => {
    try {
      const { bulkOrders, orders: ordersTable, enterpriseOrderAllocations } = await import("@shared/schema");
      ...
      try {
        await db.transaction(async (tx) => {
          ...
          if (lockedOrders.length < employeeEmails.length) {
            throw new Error(`Only ${lockedOrders.length} unallocated eSIMs available, but ${employeeEmails.length} requested`);
          }
          ...
          const insertedAllocations = await tx.insert(enterpriseOrderAllocations)
            .values(allocationsToInsert)
            .onConflictDoNothing()
            .returning();

          if (insertedAllocations.length < allocationsToInsert.length) {
            throw new Error(`CONFLICT: Only ${insertedAllocations.length} of ${allocationsToInsert.length} allocations were created due to conflicts`);
          }
        ...
        if (txError.message.includes("CONFLICT:") || txError.code === '23505') {
          return res.status(409).json({ success: false, message: "Some eSIMs were already allocated. Please refresh and try again." });
        }
        throw txError;
      }
      ...
      for (const allocation of reservedAllocations) {
        ...
        try {
          ...
          await db.update(enterpriseOrderAllocations)
          ...
          await db.delete(enterpriseOrderAllocations)
            .where(and(
              eq(enterpriseOrderAllocations.orderId, orderId),
              eq(enterpriseOrderAllocations.employeeEmail, employeeEmail)
            ));

          distributionResults.push({ email: employeeEmail, status: "failed", orderId, error: emailError.message });
        }
      }
  ...
  // Export eSIMs as CSV
  app.get("/api/enterprise/orders/:id/export", requireEnterpriseAuth, async (req: Request, res: Response) => {
    try {
      const { bulkOrders, orders: ordersTable } = await import("@shared/schema");
      ...
      if (completedOrders.length === 0) {
        return res.status(400).json({ success: false, message: "No completed eSIMs available for export" });
      }

      const csvHeader = "Order ID,ICCID,Activation Code,SMDP Address,Data Amount,Validity,QR Code URL,Status\n";
      ...
      res.setHeader('Content-Disposition', `attachment; filename="esims-order-${bulkOrder.id}.csv"`);
      res.send(csv);
    } catch (error: any) {
  ...
  app.post("/api/vouchers/validate", async (req: Request, res: Response) => {
    try {
      ...
      if (voucher.maxUses && voucher.currentUses >= voucher.maxUses) {
        return res.status(400).json({ success: false, message: "Voucher has reached maximum uses" });
      }
  ...
  app.get("/api/admin/analytics/overview", requireAdmin, async (req: Request, res: Response) => {
    try {
      const { orders: ordersTable, users: usersTable, analyticsEvents } = await import("@shared/schema");
      ...
      }).from(analyticsEvents).where(gte(analyticsEvents.createdAt, thirtyDaysAgo));
      ...
      const conversionRate = visitorsResult[0]?.count > 0
        ? (purchasesResult[0]?.count / visitorsResult[0]?.count * 100).toFixed(2)
        : 0;

      // Average order value
      const avgOrderValue = purchasesResult[0]?.count > 0
  ...
  app.get("/api/admin/analytics/funnel", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const packageViews = await db.select({
        count: sql<number>`COUNT(DISTINCT ${analyticsEvents.sessionId})`,
      }).from(analyticsEvents).where(eq(analyticsEvents.eventType, "package_view"));

      const checkoutStarts = await db.select({
        count: sql<number>`COUNT(DISTINCT ${analyticsEvents.sessionId})`,
      }).from(analyticsEvents).where(eq(analyticsEvents.eventType, "checkout_start"));

      const purchases = await db.select({
        count: sql<number>`COUNT(DISTINCT ${analyticsEvents.sessionId})`,
      }).from(analyticsEvents).where(eq(analyticsEvents.eventType, "purchase"));
  ...
  // Get abandoned carts
  app.get("/api/admin/analytics/abandoned-carts", requireAdmin, async (req: Request, res: Response) => {
    try {
  ...
  // Delete automation
  app.delete("/api/admin/email/automations/:id", requireAdmin, async (req: Request, res: Response) => {
    try {
      const { emailAutomations } = await import("@shared/schema");

      const [deleted] = await db.delete(emailAutomations)
        .where(eq(emailAutomations.id, req.params.id))
        .returning();
  ...
  // Toggle automation
  app.post("/api/admin/email/automations/:id/toggle", requireAdmin, async (req: Request, res: Response) => {
    try {
      const { emailAutomations } = await import("@shared/schema");
  ...
  app.get("/api/admin/master-regions", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const airaloCountsResult = await db.execute(sql`
        ...
        GROUP BY region_id
      `);
      const airaloCounts = new Map((airaloCountsResult.rows as any[])?.map(r => [r.region_id, r.count]) || []);
      ...
      const esimAccessCountsResult = await db.execute(sql`
        ...
        GROUP BY region_id
      `);
      const esimAccessCounts = new Map((esimAccessCountsResult.rows as any[])?.map(r => [r.region_id, r.count]) || []);
      ...
      const esimGoCountsResult = await db.execute(sql`
        ...
        GROUP BY region_id
      `);
      const esimGoCounts = new Map((esimGoCountsResult.rows as any[])?.map(r => [r.region_id, r.count]) || []);
      ...
      const mayaCountsResult = await db.execute(sql`
        ...
        GROUP BY region_id
      `);
      const mayaCounts = new Map((mayaCountsResult.rows as any[])?.map(r => [r.region_id, r.count]) || []);
      ...
      const regionsWithCounts = regionsData.map(region => ({
        ...
        packageCounts: {
          ...
          total: (airaloCounts.get(region.id) || 0) + (esimAccessCounts.get(region.id) || 0) + (esimGoCounts.get(region.id) || 0) + (mayaCounts.get(region.id) || 0),
        }
      }));
  ...
  app.get("/api/admin/master-countries", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      `);
      const airaloCounts = new Map((airaloDestCountsResult.rows as any[])?.map(r => [r.destination_id, r.count]) || []);
      ...
      const esimAccessDestCountsResult = await db.execute(sql`
        ...
        GROUP BY destination_id
      `);
      const esimAccessCounts = new Map((esimAccessDestCountsResult.rows as any[])?.map(r => [r.destination_id, r.count]) || []);
      ...
      const esimGoDestCountsResult = await db.execute(sql`
        ...
        GROUP BY destination_id
      `);
      const esimGoCounts = new Map((esimGoDestCountsResult.rows as any[])?.map(r => [r.destination_id, r.count]) || []);
      ...
      const mayaDestCountsResult = await db.execute(sql`
        ...
        GROUP BY destination_id
      `);
      const mayaCounts = new Map((mayaDestCountsResult.rows as any[])?.map(r => [r.destination_id, r.count]) || []);
      ...
      const destinationsWithCounts = destinationsData.map(dest => ({
        ...
        packageCounts: {
          ...
          total: (airaloCounts.get(dest.id) || 0) + (esimAccessCounts.get(dest.id) || 0) + (esimGoCounts.get(dest.id) || 0) + (mayaCounts.get(dest.id) || 0),
        }
      }));
      ...
      ApiResponse.success(res, "Countries fetched successfully", { destinations: destinationsWithCounts, stats });
    } catch (error: any) {
  ...
  app.patch("/api/admin/master-countries/:id", requireAdmin, async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const { image, active } = req.body;
  ...
  app.post("/api/admin/master-countries/sync", requireAdmin, async (req: Request, res: Response) => {
    try {
  ...
  // Upload region image
  app.post("/api/admin/master-regions/:id/upload-image", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      regionImageUpload.single("image")(req, res, async (err: any) => {
        try {
          ...
          ApiResponse.success(res, "Image uploaded successfully", { image: imageUrl, region: updated });
  ...
  app.post("/api/admin/master-countries/:id/upload-image", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      countryImageUpload.single("image")(req, res, async (err: any) => {
        try {
          ...
          ApiResponse.success(res, "Image uploaded successfully", { image: imageUrl, destination: updated });
        } catch (innerError: any) {
  ...
  app.post("/api/admin/languages", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      if (!parsed.success) {
        return res.status(400).json({ success: false, message: "Invalid language data", errors: parsed.error.errors });
      }
  ...
  app.post("/api/admin/languages/:id/set-default", requireAdmin, async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
  ...
  app.post("/api/admin/translations/keys", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      if (!parsed.success) {
        return res.status(400).json({ success: false, message: "Invalid key data", errors: parsed.error.errors });
      }
  ...
  app.delete("/api/admin/translations/keys/:id", requireAdmin, async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
  ...
  app.get("/api/admin/translations/:languageId", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const translationMap = new Map(translations.map(t => [`${t.namespace}.${t.key}`, t.value]));
  ...
  app.put("/api/admin/translations/:languageId/:keyId", requireAdmin, async (req: Request, res: Response) => {
    try {
  ...
  app.get("/api/admin/translations/:languageId/missing", requireAdmin, async (req: Request, res: Response) => {
    try {
      const { languageId } = req.params;
  ...
  app.post("/api/admin/translations/:languageId/import", requireAdmin, async (req: Request, res: Response) => {
    try {
      const { languageId } = req.params;
  ...
  app.get("/api/admin/translations/:languageId/export", requireAdmin, async (req: Request, res: Response) => {
    try {
      const { languageId } = req.params;
      ...
      if (format === "json") {
        ...
        res.setHeader("Content-Disposition", `attachment; filename="${language.code}_translations.json"`);

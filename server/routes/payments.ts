...
import { initPaypalPayment } from '../helpers/payments/paypal';
import { initPaystackPayment } from '../helpers/payments/paystack';
import { initPowertranzSpiSale } from '../helpers/payments/powertranz';
import { requireAuth, optionalAuth } from 'server/lib/middleware';
import { calculateFinalPrice } from 'server/helpers/calculatePricing';
import verifyPaypal from 'server/helpers/payments/verify/paypal';
import verifyPaystack from 'server/helpers/payments/verify/paystack';
...
/* ======================================================
   GET: Enabled payment gateways (User)
====================================================== */
...
//       provider: paymentGateways.provider,
//       displayName: paymentGateways.displayName,
//       publicKey: paymentGateways.publicKey, // ✅ SAFE
//     })
//     .from(paymentGateways)
//     .where(eq(paymentGateways.isEnabled, true))
//     .orderBy(
...
router.get('/gatewaysolll', async (req, res) => {
  const currencyCodes = req.query.currency as string | undefined;

  // 🚫 currency is mandatory
  if (!currencyCodes) {
    return res.status(400).json({
      success: false,
      message: 'currency query param is required',
      example: '/gateways?currency=USD',
    });
  }
  ...
  const [currency] = await db
    ...
    .from(currencyRates)
    .where(eq(currencyRates.code, currencyCodes));
  ...
  const gateways = await db
    ...
    .where(and(eq(paymentGateways.isEnabled, true), eq(supportedCurrency.currencyId, currency.id)))
...
router.post('/inittt', optionalAuth, async (req, res) => {
  try {
    const {
      gatewayId,
      packageId,
    ...
    const pricing = await calculateFinalPrice({
      ...
      giftCardId,
      referralCredits,
      userId: req.userId, // undefined for guest
    });
    ...
    switch (gateway.provider) {
      ...
      case 'paystack':
        if (!email) {
          return res.status(400).json({
            success: false,
            message: 'Email required for Paystack',
          });
        }
...
router.post('/init', optionalAuth, async (req, res) => {
  try {
    ...
    switch (gateway.provider) {
      ...
      //   result = await initStripePayment({
      //     secretKey: gateway.secretKey!,
      //     amount: pricing.total, // major unit
      //     currency,
      //     packageId,
      //     quantity,
      //     orderId,
...
//     switch (provider) {
//       case "stripe":
//         verificationResult = await verifyStripe(req.body);
//         break;

//       case "razorpay":
//         verificationResult = await verifyRazorpay(req.body);
//         break;

//       case "paypal":
//         verificationResult = await verifyPaypal(req.body);
//         break;

//       case "paystack":
//         verificationResult = await verifyPaystack(req.body);
//         break;
...
//     // ✅ Payment verified → order create / fulfill
//     return res.json({
//       success: true,
...
router.post('/initold', requireAuth, async (req, res) => {
  try {
    const { gatewayId, amount, currency, orderId, email, name } = req.body;
